...
;(function() {
	...
	//ice版本号
	ice.version = '2.0.2';
	ice.site = 'https://www.iceui.cn';
	navigator.userAgent.toUpperCase().indexOf('MSIE')==-1 ? console.log.apply(console, [
		'%c %c %c %c　ICEUI '+ice.version+' - ★ 飘飘兮若回风之流雪 ★  %c  %c %c   '+ice.site+'  %c ❤ %c❤ %c❤',
		...
		'color:#8783e1;background:#fff; padding:2px 0;font-size:12px;',
		'color:#8783e1;background:#fff; padding:2px 0;font-size:12px;'
	]) : console.log('ICEUI '+ice.version+' - 飘飘兮若回风之流雪 - '+ice.site);
	//ice链接地址
			...
			xhr.onreadystatechange = function() {
				if (xhr.readyState == 4) {
					...
					if (xhr.status >= 200 && xhr.status < 300 || xhr.status == 304) {
						...
						json.complete && json.complete(xhr.status);
					}
				}
			}
		})
    ...
    ice.get = function (options, data, success, error, complete) {
        ...
        json.complete = function (status) {
            completeFn && completeFn(status);
        };
        if (!json.url) return;
        json.type = 'get';
    ...
    ice.pjax = function (json,push) {
		...
		//加载中动画
		var load,loadTime,width=300,s=30;
		if(json.loading){
			//加载动画的html
    ...
    ice.keydown = function(key,callback){
	    document.onkeydown = function (e) {
	        ...
	        if(typeof key == 'function'){
	        	key.call(theEvent,code);
	        }else{
		        if (code == key) {
		        	if(typeof callback == 'string'){
		        		eval(callback);
		        	}else{
		        		callback && callback(code,e);
		        	}
	...
	ice.randStr = function(num) {
		...
		while (num--) {
			...
			str += Math.random()>0.4?String.fromCharCode(ice.getRandom(65,90)):String.fromCharCode(ice.getRandom(48,57));
		}
		return str;
	};
	//跳转网页
